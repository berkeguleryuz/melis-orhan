  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="w-6 h-6 border-2 border-[#c75050]/30 border-t-[#c75050] rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {wedding.hasGallery && (
        ...
        >
          <div className="bg-white rounded-3xl border border-[#1a1210]/[0.06] shadow-sm p-4">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
              <div className="relative flex-1">
                <UserIcon
                  size={16}
                  className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[#c75050]/55"
                />
                <input
                  type="text"
                  value={uploaderName}
                  ...
                  className="w-full h-11 rounded-xl border border-[#1a1210]/10 bg-[#faf0ea]/60 pl-9 pr-4 text-base sm:text-sm text-[#1a1210] placeholder:text-[#1a1210]/40 focus:border-[#c75050] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#c75050]/15 transition-all font-sans"
                />
              </div>
              ...
              <button
                ...
                className={cn(
                  "h-11 px-5 rounded-xl font-sans text-xs font-semibold tracking-wide flex items-center justify-center gap-2 transition-all shrink-0",
                  isUploading
                    ? "bg-[#c75050]/20 text-[#c75050]/50 cursor-not-allowed"
                    : "bg-gradient-to-r from-[#c75050] to-[#d4898a] text-white hover:opacity-90 cursor-pointer"
                )}
              >
                {isUploading ? (
                  <>
                    <div className="w-3.5 h-3.5 border-2 border-[#c75050]/30 border-t-[#c75050] rounded-full animate-spin" />
            ...
            <p className="font-sans text-[10px] text-[#1a1210]/30 mt-2 text-center">
              {t("galleryUploadInfo")}
            </p>
          </div>
        </motion.div>
      ...
      {photos.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 2xl:grid-cols-8 gap-2">
          {photos.map((photo, index) => (
            <motion.div
              ...
              className="group relative aspect-[4/3] overflow-hidden rounded-2xl border border-[#1a1210]/[0.08] cursor-pointer bg-transparent hover:shadow-[0_8px_30px_rgba(167,139,250,0.15)] transition-shadow duration-300"
        ...
        >
          <div className="text-center py-16 space-y-3">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#c75050]/10 to-[#d4898a]/10 border border-[#c75050]/10 flex items-center justify-center mx-auto mb-4">
              <CameraIcon className="size-6 text-[#c75050]/50" size={24} />
            </div>
            <p className="font-sans text-sm text-[#1a1210]/50">
              {t("galleryEmpty")}
            </p>
            <p className="font-sans text-xs text-[#1a1210]/30">
      ...
      <AnimatePresence>
        {selectedPhoto && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-[#1a1210]/95 backdrop-blur-sm flex items-center justify-center p-4"
